...
#### **頁面標題慣例**
**重要規範：編輯頁的主標題必須與側邊欄名稱保持一致**

```javascript
// ❌ 錯誤：區分編輯/新增
const pageTitle = isEditing ? '編輯 OAuth 客戶端' : '新增 OAuth 客戶端'
...
**標題對應規則：**
- 側邊欄：「OAuth 客戶端」→ 編輯頁標題：「OAuth 客戶端」
- 側邊欄：「參數設定」→ 編輯頁標題：「參數設定」
- 側邊欄：「權限定義」→ 編輯頁標題：「權限定義」
- 側邊欄：「角色管理」→ 編輯頁標題：「角色管理」

**設計理念：**
- 保持介面一致性，用戶能清楚知道當前所在的功能模組
- 標題簡潔明瞭，不需要重複「編輯」、「新增」等動作詞
- 副標題（描述文字）可以區分編輯/新增的具體動作
